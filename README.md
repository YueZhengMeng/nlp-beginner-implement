...
2. 手搓BOW与NGram两种tokenizer，在训练集上训练词表  
    为了降低手搓模型的复杂度与工程量，我选择将句子tokenize为multi-hot向量，经过线性层映射后直接得到句向量，而不是使用词向量。该实现方法不需要PAD与UNK这两个special token。代码见 ./lab1/tokenizers.py
3. 拟合实验：
    使用训练集的前3000条数据训练模型，使用训练集的3000-3999条数据验证模型，进行100个epoch的训练。  
    基于BOW与NGram的模型在训练集上最终都可以达到97%以上的准确率，在验证集上只需要1个epoch就可以达到56%左右的准确率，但是之后不会继续上升，10个epoch之后还会下降。    
    实验结果见 ./lab1/exp 文件夹下文件名包含ds=3000的图片和csv文件。
4. 正式训练：
    使用训练集的前150000条数据训练模型，使用训练集的后6060条数据验证模型。基于拟合实验的结论，只进行10个epoch的训练。保存验证集精度最高的模型，对测试集进行预测。
    实验结果见 ./lab1/exp 文件夹下文件名包含ds=150000的图片和csv文件。
5. 上传kaggle：
...
2. 增加batch_size并等比例增加学习率可以加快训练速度，但是最终精度会下降。实测batch_size=32比较合适。
...
&emsp;./lab1/math_formula.ipynb中，交叉熵损失函数的反向传播 这一章节部分公式因大小括号不匹配导致显示错误，现已修正。  

## 更新
&emsp;加入自制AdamW优化器。  
&emsp;同样进行了小规模实验，探索最佳的超参数。实测发现使用1e-3的初始学习率，并随训练过程线性下降到1e-5最佳。  
&emsp;小规模实验结果与使用SGD优化器时相同：基于各种tokenizer的模型均表现出较强的数据拟合能力，可以在训练集上达到将近100%的精度，但普遍存在泛化性能不足的问题——验证集上的精度在迅速接近60%后，就不再上升。  
&emsp;但是相比于SGD优化器，AdamW优化器在训练后期，没有明显的验证集精度下降。该现象也可能与远低于SGD优化器的学习率有关。  
&emsp;正式实验转移到AutoDL平台租赁的CPU服务器上进行。原因是本次实验完全基于Numpy实现，导致自实现的AdamW优化器训练极为缓慢——原本SGD优化器仅需半小时的正式实验训练过程，AdamW则需要约十小时才能完成。  
&emsp;提交kaggle评测，基于BOW的模型，精度为58.361%；基于NGram的模型，精度为58.645%；基于BPE的模型，精度为59.337%；  
&emsp;截图见 ./lab1/exp/kaggle_result_lab1_adamw.png  
&emsp;分析实验结果可以发现，基于NGram的模型与其他两款模型的差距明显缩小，这是因为AdamW优化器能够为不频繁更新的、稀疏的参数动态分配较高的学习率，从而有效促进其收敛。  
&emsp;综合来看，基于BPE tokenizer的模型在所有设定中表现最佳。  
&emsp;关于不同优化器之间的性能差异，由于当前模型结构较为简单，训练过程中的随机因素影响较大，难以得出明确结论。  

## 更新
&emsp;使用`\`换行替代`双空格`换行。解决ipynb文件中的Markdown块预览后自动修剪行末多余空格导致换行失效的问题。
