...
5. 以9:1的比例划分训练集和测试集，进行训练。  
   ...
   保存困惑度最低的模型，用于之后的生成。  
6. 实验结果：  
   使用诗句进行训练，模型在训练集上可以充分拟合，困惑度最低1.1123，next token accuracy最高为99.98%  
   但是测试集上的困惑度在第3个epoch后达到最低，为2093.00，next token accuracy为0.72%，几乎没有任何泛化能力。此时训练集上的困惑度为1120.1222，next token accuracy为0.79%，还没有收敛。    
   之后随着训练集上的困惑度下降，测试集上的困惑度反而快速上升，最终超过90000。  
   使用充分拟合训练集的模型进行推理测试，对于训练集中的诗句，可以准确生成后半句，但是EOS的位置不准确，导致生成的诗句之后会有一段无意义的诗句。这可能与teacher forcing训练机制有关。  
   对于测试集中的诗句，生成的诗句无意义。  
   使用测试集上困惑度最低的模型进行推理测试，生成的序列是一系列相同的字，无意义。    
&emsp;  
   使用完整诗句进行训练，模型在训练集上都无法收敛，困惑度最低为16.4957，next token accuracy最高为60.09%  
   在测试集上的困惑度在第6个epoch后达到最低，为1794.5411，next token accuracy为1.00%，几乎没有任何泛化能力。此时训练集上的困惑度为818.2307，next token accuracy为0.725%，还没有收敛。      
   之后随着训练集上的困惑度下降，测试集上的困惑度反而快速上升，最终超过100000。  
   使用充分拟合训练集的模型进行推理测试，对于训练集或测试集中的诗句，再生成几个字就会陷入重复生成的循环，无法生成有意义的诗句。  
   使用测试集上困惑度最低的模型进行推理测试，生成的序列是一系列相同的字，无意义。
